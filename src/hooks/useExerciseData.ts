...
export const useExerciseData = () => {
  ...
  const [recentExercises, setRecentExercises] = useState<Exercise[]>(RECENT_EXERCISES);

  // Function to simulate saving a new exercise entry
  const addExerciseEntry = useCallback((name: string, duration: number, calories: number) => {
    ...
    setRecentExercises(prev => 
      [newEntry, ...prev].slice(0, 5) // Keep last 5 recent, for example
